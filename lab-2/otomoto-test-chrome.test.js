...
describe('Otomoto', () => {
    ...
    test('Should open car list by selected brand on homepage', async () => {
        ...
        expect(await input.getAttribute('value')).toBe(brandWithNumbers);
        ...
        const firstCarItem = await driver.wait(
            until.elementLocated(By.css('h1.epwfahw9.ooa-1ed90th.er34gjf0 > a')),
            10000
        );
        ...
        await driver.findElement(By.css('article.ooa-yca59n')).click();
        const buttonShowNumber = await driver.wait(
            until.elementLocated(By.css('button.evok7j41.ep2wx1j0.ooa-1ixjak3')),
            4000
        );
        await driver.executeScript("arguments[0].click();", buttonShowNumber);
    }, 15000);
});
